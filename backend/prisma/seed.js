...
const seedUsers = [
    { name: 'Admin User', email: 'admin@example.com', role: client_1.Role.ADMIN },
    { name: 'Sales User', email: 'sales@example.com', role: client_1.Role.SALES },
    { name: 'Warehouse User', email: 'warehouse@example.com', role: client_1.Role.WAREHOUSE },
    { name: 'Accounts User', email: 'accounts@example.com', role: client_1.Role.ACCOUNTS },
];
const firstNames = [
...
async function runDatabaseSeed() {
    ...
    const passwordHash = await (0, password_1.hashPassword)(INITIAL_PASSWORD);
    // 1. Seed System Users
    const createdUsers = [];
    ...
    const adminUser = createdUsers.find((u) => u.role === client_1.Role.ADMIN) || createdUsers[0];
    const salesUser = createdUsers.find((u) => u.role === client_1.Role.SALES) || createdUsers[0];
    const warehouseUser = createdUsers.find((u) => u.role === client_1.Role.WAREHOUSE) || createdUsers[0];
    const accountsUser = createdUsers.find((u) => u.role === client_1.Role.ACCOUNTS) || createdUsers[0];
    // 2. Seed 100 Customers
    console.log('Creating 100 Customers...');
    const customerTypes = [client_1.CustomerType.RETAIL, client_1.CustomerType.WHOLESALE, client_1.CustomerType.DISTRIBUTOR];
    const customerStatuses = [client_1.CustomerStatus.ACTIVE, client_1.CustomerStatus.LEAD, client_1.CustomerStatus.INACTIVE];
    ...
    for (let i = 1; i <= 100; i++) {
        ...
        const status = customerStatuses[i % 3 === 0 ? 1 : i % 5 === 0 ? 2 : 0]; // Mostly ACTIVE & LEAD
        ...
        const customer = await prisma.customer.upsert({
            ...
            create: {
                ...
                address: `Plot #${10 + i}, Industrial Zone, ${city}`,
                status,
                followUpDate: status === client_1.CustomerStatus.LEAD ? new Date(Date.now() + i * 86400000) : null,
    ...
    // 4. Seed 50 Sales Challans with Line Items
    console.log('Creating 50 Sales Challans...');
    const challanStatuses = [client_1.ChallanStatus.CONFIRMED, client_1.ChallanStatus.DRAFT, client_1.ChallanStatus.CANCELLED];
    for (let i = 1; i <= 50; i++) {
    ...
    for (let i = 1; i <= 50; i++) {
        ...
        await prisma.stockMovement.create({
            data: {
                ...
                quantity: qty,
                type,
                reason: type === client_1.StockMovementType.IN ? `Bulk Inward Shipment PO #${8000 + i}` : `Dispatched for Sales Order CH-2026-${1000 + i}`,
                createdById: warehouseUser.id,
            },
        });
    }
    console.log('✅ Seeded 50 Stock Movements.');
    // 6. Seed 30 Inter-Department Approval Requests
    ...
    const targetRoles = [client_1.Role.WAREHOUSE, client_1.Role.ACCOUNTS, client_1.Role.SALES, client_1.Role.ADMIN];
    const reqStatuses = [client_1.RequestStatus.PENDING, client_1.RequestStatus.APPROVED, client_1.RequestStatus.REJECTED];
    for (let i = 1; i <= 30; i++) {
        ...
        const reviewer = status !== client_1.RequestStatus.PENDING ? (targetRole === client_1.Role.ADMIN ? adminUser : accountsUser) : null;
        await prisma.approvalRequest.create({
            data: {
                ...
                reviewNote: status === client_1.RequestStatus.APPROVED ? 'Approved after review by department lead.' : status === client_1.RequestStatus.REJECTED ? 'Declined due to policy/credit limits.' : null,
            },
        });
    }
    console.log('✅ Seeded 30 Inter-Department Approval Requests.');
    console.log('🎉 Bulk Database Seeding Completed Successfully! Total ~300+ entries generated.');
}
